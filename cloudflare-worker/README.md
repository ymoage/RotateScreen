...
拡張機能のオプション画面で選択したモデルがWorkerにも適用されます。

- 拡張機能で「Gemini API直接」モードを選択
- 🔄ボタンでモデル一覧を取得
- 使用したいモデルを選択

選択したモデルはWorker経由でも使用されます（再デプロイ不要）。

> **補足**: Cloudflare Dashboardで環境変数 `GEMINI_MODEL` を設定すると、拡張機能からモデル指定がない場合のデフォルトとして使用されます。
