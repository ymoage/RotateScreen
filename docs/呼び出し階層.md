...
2. [Face Detector (content/face-detector.js)](#face-detector)
3. [Background Service Worker (background/service-worker.js)](#background-service-worker)
4. [Popup (popup/popup.js)](#popup)
5. [Options (options/options.js)](#options)
6. [Settings Manager (storage/settings-manager.js)](#settings-manager)
7. [コンポーネント間通信](#コンポーネント間通信)
...
│   │   └── updateButtonState()
│   └── tryAutoRotation()             # 自動回転検出（有効時）
│       ├── isLiveStream()            # ライブ配信チェック
│       └── window.RotateScreenFaceDetector.detectVideoOrientation()
│           └── showAutoRotationNotification()
...
```
detectVideoOrientation(video)         ← Content Scriptから呼び出し
├── createFaceDetector()              # FaceDetector APIインスタンス作成
│   └── new FaceDetector()
├── detectByFaceAPI()                 # FaceDetector APIで検出（優先）
│   ├── detector.detect()
│   ├── analyzeFromLandmarks()        # ランドマークから向き分析
│   │   └── Math.atan2()              # 目→口ベクトルの角度計算
│   └── バウンディングボックス分析    # ランドマークがない場合
└── detectByColorAnalysis()           # 肌色分析（フォールバック）
    ├── canvas.getContext('2d')
    ├── ctx.drawImage()               # 動画フレームを描画
    ├── ctx.getImageData()            # ピクセルデータ取得
    ├── isSkinColor()                 # 各ピクセルの肌色判定
    │   └── 色相・彩度・明度チェック
    └── 領域比較                       # 上下左右の肌色比率比較
```
