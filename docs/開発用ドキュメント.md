...
```javascript
{
  ...
  autoDetectRotation: boolean  // 顔検出による自動回転（実験機能、デフォルトOFF）
}
```
...
動画内の顔や肌色を検出して、動画の向きを自動判定する機能です。デフォルトでは無効になっています。

### 検出方法

1. **FaceDetector API**（優先）
   - Chrome実験機能として提供
   - `chrome://flags/#enable-experimental-web-platform-features` で有効化が必要
   - 高精度だがブラウザサポートが限定的

2. **肌色分析**（フォールバック）
   - FaceDetector APIが利用できない場合に使用
   - 動画フレームを4分割して肌色分布を分析
   - 肌色が多い領域を頭部と推定
...
動画読み込み
    │
    ▼
ライブ配信チェック ─── YES ──→ 自動回転スキップ
    │
    NO
    ▼
保存済み回転チェック ─── あり ──→ 保存済み回転を適用
    │
    なし
    ▼
FaceDetector API利用可能？ ─── YES ──→ FaceDetector APIで検出
    │                                      │
    NO                                     ▼
    │                               検出成功？
    ▼                                  │
...
- ライブ配信では自動回転が無効
- 検出精度は限定的（誤検出の可能性あり）
- 動画の最初の数フレームで判定するため、冒頭が暗い動画では検出困難
